...
There is intentionally no `lfx[all]` extra; use `langflow` for the server plus default curated bundles, and install `lfx-bundles` explicitly for the long-tail providers. Torch-gated extras stay opt-in even then: `pip install "lfx-bundles[all]"` adds the `cuga`/`codeagents` components, and `pip install "langflow[docling]"` adds the File component's docling conversion path.
