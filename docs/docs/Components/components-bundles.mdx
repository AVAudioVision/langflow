...
`uv pip install langflow` includes the default curated standalone provider bundles automatically.
Long-tail providers in `lfx-bundles` must be installed explicitly.
...
The default `uv pip install langflow` stays torch-free. The long-tail **CUGA** and **Code Agents** bundles aren't installed by default, and the **Docling** local parser remains opt-in.

`uv pip install "lfx[bundles]"` installs all long-tail providers, including CUGA and Code Agents and their PyTorch dependencies.

To add all long-tail providers to a Langflow environment, run `uv pip install "lfx-bundles[all]"`. To enable the Docling local parser, run `uv pip install "langflow[docling]"`.
